...
public class ChessBoard extends JFrame implements Observer {
	...
	private void setUpTool() {
        ...
        graph = createButton("Graph");

        tool.add(start);//I initialize the buttons
        tool.add(time);
        tool.add(setSize);
        tool.add(graph);
	}
    ...
    private void setResultToSquare(int i, int j, Color color, JButton square) {
    	UndirectedGraph<Square, DefaultEdge> knightGraph = model.getKnight().getGraph();//The graph of the tour
    	String position = Integer.toString(model.getKnight().getVertex(i, j, knightGraph).getPosition());
